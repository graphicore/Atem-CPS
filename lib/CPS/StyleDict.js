  ...
  , SelectorList
    ...
    function StyleDict(controller, element, rules /* default: null */) {
        ...
        this.getAPI = {
          ...
          /**
           * This is currently used to track the rules that created
           * User-Interface generating values. If needed,
           * `getAPI.getRuleForProperty` can be used to get the rule for
           * the current property. The rule can be attached to the value
           * and then be changed by the UI.
           *
           * References: http://atem.graphicore.de/content/Proposals/cps-ui.md.html#the-hack
           */
          , getCurrentPropertyAndRule: function() {
                var key = self._getting.current
                  , rule = self.getRuleForKey(key)
                  , index = rule.properties.getActiveIndex(key)
                  ;
                return [rule.properties.getItem(index), rule];
            }
        };
    ...
    _p._startWrappedNextTrigger = function() {
        this._nextTriggerWrappers += 1;
    };

    _p._endWrappedNextTrigger = function() {
        this._nextTriggerWrappers -= 1;
        if(this._nextTriggerWrappers === 0)
            this._delayedTriggerAll();
    };

    /**
     * The outermost decorated method will call _delayedTriggerAll
     * just before returning.
     * This makes the event emission synchronous, but delayed to the end
     * of the internal process.
     *
     * Everything that leads to a call to _nextTrigger should be wrapped
     * with this decorator, where everything ideally means the most outer
     * call that leads to a next trigger, i.e. the interface that is called.
     *
     * NOTE: it's maybe more complex. If the event emission causes
     * more calls to decorated nextTrigger methods via graph/network
     * effects. Maybe _delayedTrigger must loop until no more events are
     * created by triggering (at the risk of creating an endless loop)
     * or, when in _delayedTrigger _nextTrigger is allowed to trigger
     * immediately.
     * I was not able to observe this yet! There's a watchdog assertion
     * at the end of _delayedTrigger
     */
    function _decorateNextTrigger(method) {
        return function () {
            //jshint validthis:true
            var args = [], i, l;
            for(i=0,l=arguments.length;i<l;i++)
                args.push(arguments[i]);
            // this is the StyleDict instance
            this._startWrappedNextTrigger();
            try {
                // finally will be executed before the actual return
                // but after the call to method
                return method.apply(this, args);
            }
            finally {
                this._endWrappedNextTrigger();
            }
        };
    }
    ...
    _p._getSubscriptionUid = function(item, key) {
        ...
        if(item instanceof Instance) {
            if(key instanceof SelectorList)
                ...
                // for a Instance have the same SubscriptionUid:
    ...
    _p._nextTrigger = function(channelKey, data) {
        ...
        // FIXME: remove all of this once it stops complaining
        // when it complains, the entry method in StyleDict must be
        // decorated with _decorateNextTrigger (like _invalidateCacheHandler)
        // See the doc-string of _decorateNextTrigger.
        // Also cleanup _delayedTrigger when removing this.
        var stack = '(enable in source code)';//(new Error()).stack;
        channel.timeoutID = setTimeout(function() {
            console.error('Not wrapped _nextTrigger for channel:'
                        , '"' + channelKey + '"'
                        , 'of nodeID' , this.element.nodeID
                        , 'stack:', stack);
        }.bind(this), 0, channelKey);
    };
    ...
    _p._delayedTrigger = function(channelKey) {
        ...
        if(channelKey in this._delayedTriggerData)
            // I believe this is unwanted but absolutely possible!
            // see the NOTE in the doc-string of _decorateNextTrigger
            throw new AssertionError('_delayedTrigger should delete the channel "'
                + channelKey +'" but presumably graph/network effects '
                + 'created it again');

    };

    _p._delayedTriggerAll = function() {
        var channelKeys = Object.keys(this._delayedTriggerData)
          , i, l
          ;
        for(i=0,l=channelKeys.length;i<l;i++)
            this._delayedTrigger(channelKeys[i]);
    };

    _p._invalidateCacheHandler = _decorateNextTrigger(
    ...
    _p._subscribeTo = function(item, key) {
        ...
        if(!dependencies) {
            if(typeof item.onPropertyChange !== 'function') {
                ...
                if(key instanceof SelectorList) {
                    assert(item instanceof Instance, 'When "key" is a Selector '
                                        +'"item" must be an OMA Instance.');
     ...
     */
    _p.checkRules = _decorateNextTrigger(
    function() {
        if(!this._rules)
    ...
    _p._invalidateDict = function() {
        ...
        for(key in this._cache) {
            // Some keys are left over.
            // Only do this if  this._cache[key] is an Error
            // the other left overs seem to be properties white listed by
            // the MOM-element and `this`
            if(this._cache[key] instanceof Error)
                this._invalidateCache(key);
        }
     ...
     * needed.
     */
    _p.invalidateRules = _decorateNextTrigger(
    function() {
        this._setRules(null);
    });
    ...
    _p._propertyUpdateHandler = _decorateNextTrigger(
    function(data, channelKey, keys) {
        //jshint unused:false
     ...
     */
    _p._propertyAddHandler = _decorateNextTrigger(
    function(data, channelKey, keys) {
        var i, l;
    ...
    _p.__propertyAddHandler = function(data, channelKey, key) {
        ...
        // indexes come first.
        if (currentRuleIndex === undefined) {
            this._nextTrigger('change', key);
            if(key in this._cache) {
                // this must be a cached key error from a previous
                // failed request to this StyleDict for key
                this._invalidateCache(key);
            }
        }
    ...
    _p._propertyChangeHandler = _decorateNextTrigger(
    function(properties, key, eventData) {
        switch(eventData) {
            case('change'):
     ...
     */
    _p.getPropertyValue = function(key) {
        if(!this._dict) this._buildIndex();
    ...
    _p.getRuleForKey = function(key) {
        var ruleIndex;
        if(!this._dict) this._buildIndex();
        if(!(key in this._propertySubscriptions))
            return null;
        ruleIndex = this._propertySubscriptions[key][2];
        return this._rules[ruleIndex][1];
    };

    _p.__get = function(key, errors) {
        var property = this.getPropertyValue(key)
          , result
          ;
        if(property) {
            if(typeof property.value.evaluate !== 'function'){
                console.warn('property.value.evaluate is not a function');
                console.log(key, property, this.element);

            }
            result = property.value.evaluate(this.getAPI);
