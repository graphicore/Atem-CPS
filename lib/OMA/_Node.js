  ...
  , bloomfilter
    ...
    _p.remove = function(item) {
        ...
        for(i=0,l=this._children.length;i<l;i++) {
            ...
            // All children from idx need now a new index!
            if(i >= idx) {
                data = this._childrenData.get(child);
                data[0] = i;
            }
        ...
        // FIXME: if different, the key that is triggered here should be
    ...
    _p.add = function(item) {
        ...
        id = this._children.length;
        this._children.push(item);
        this._childrenData.set(item, [id, subscription]);
        item.parent = this;
        // do this to trigger tail-index changes
        // no need to do so for the just added child (it just updated it's
        // index in the parent setter)
        for(i=0,l=this._children.length-1;i<l;i++)
            this._children[i].updateIndex();

        // A Node must trigger propertyChange(subtree) when it manipulates
        // its children list (that's why it doesn't listen to "index")
        // The "subtree" event is used in StyleDict, to get noticed when a
        // SelectorList-key potentially needs re-evaluation.
        // Thus, this is the root of all "subtree" property change events.
        this._triggerPropertyChange(['children', 'subtree']);
    };
