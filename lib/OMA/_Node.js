define([
  ...
  , emitterMixin
    ...
    // the OMAController managing the instance
    // is set in OMAController._initPattern
    _p.omaController = undefined;

    // shortcut to omaController
    _p.createPattern = function(typeName) {
        return this.omaController.createPattern(typeName);
    };
    ...
    Object.defineProperty(_p, 'makeProperty', {
        get: function() {
            ...
            throw new NotImplementedError('Implement makeProperty!');
        }
    });
    ...
    _p.getChild = function(index) {
        return this.getEdge(index).to;
    };

    _p.getEdgeByInstance = function(instance) {
        var edge = this._edges[instance.index];
        assert(edge.instances.has(instance)
                                        , 'Instance must be registered');
        return edge;
    };

    _p.getEdge = function(index) {
        assert(!!this._edges[index], 'Edge must exist: ' + index
                                    + '; # eges: '+ this._edges.length);
    ...
    function _getCanonicalStartIndex(start, length) {
        ...
        if(start >= length)
            return length;
        if(start < 0)
            return Math.max(0, length - (start + 1));
        return start;
    }
    ...
    _p.insertAt = function(startIndex,  insertions /* single item or array of items */) {
        if(this.isFrozen)
            throw new OMAError('Inserting children is not allowed in this element.');

        var insertions_ = insertions instanceof Array
                ? insertions
                : (insertions === undefined ? [] : [insertions])
          , canonicalStartIndex = _getCanonicalStartIndex(startIndex, this._edges.length)
          , notAccepted, args
          ;
        if(!insertions_.length)
            return;

        notAccepted = insertions_.filter(function(item) {
            //jshint validthis: true
        ...
        insertions_ = insertions_.map(to => Edge.factory(this, to));
        ...
        this._trigger('inserted', [canonicalStartIndex, insertions_.length]);
        this._essenceChanged();// will call this._trigger('essence')
        return canonicalStartIndex;
    };
    ...
    _p.removeAt = function(startIndex, deleteCount) {
        ...
        deleted.map(edge=>edge.to)
               .reduce((accumulator, deleted)=>{
                   if(!childrenSet.has(deleted)) // filter
     ...
     */
    _p._cps_getters = {};
    _p.cpsHasFromInstance = function(instance, key) {
        return whitelisting.has(this._cps_getters, this, key);
    };

    /**
     * This is used either to give names to children of $frozenChildren
     * nodes or to make ancestor nodes easier accessible (seen in MOM with
     * multivers/univers/master/glyph)
     * Both should be configurable here AND integrate with cpsGet in the
     * instance, i.e. behave like whitelisting.getMethod.
     * The reason is that Instances shouldn't be specialized for the
     * OMA-implemenataion, so we can trust they always behave the same,
     * while the nodes implement the model specifics.
     */
    _p.cpsGetFromInstance = function(instance, key) {
        var [found, result] = whitelisting.checkGetters(
                                            this._cps_getters, this, key)
            , from, method, args, target
            ;
        if(!found)
            throw new KeyError(result);

        [from, method, ...args] = result;
        switch(from) {
            case 'instance':
                target = instance;
                break;
            case 'this':
                // add instance to the getter args to make it more versatile
                // otherwise, we don't know the instance there.
                args.unshift(instance);
                target = this;
                break;
            default:
                throw new OMAError('Bad name for a target: "' + from + '"');
        }
