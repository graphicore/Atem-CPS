define([
  ...
  , bloomfilter
    ...
    /**
     * cloneFlags: Array of booleans:
     * [
     *      cloneIdAndClasses ,
     *      cloneElementProperties ,
     *      cloneAttachedData
     *  ]
     * or a bitmask (integer number) with the flags:
     *      cloneIdAndClasses      = 1
     *      cloneElementProperties = 2
     *      cloneAttachedData      = 4
     *
     */
    _p.clone = function(cloneFlags) {
        var clone = new this.constructor(), i,l;
        this._cloneProperties(clone, cloneFlags);
        for(i=0,l=this._children.length;i<l;i++)
            clone.add(this._children[i].clone(cloneFlags));
        return clone;
    };
    ...
    _p._cloneProperties = function(clone, cloneFlags) {
        var isArray = Array.isArray(cloneFlags)
          , cloneIdAndClasses = isArray
                        ? cloneFlags[0]
                        : (cloneFlags & 1)
          , cloneElementProperties = isArray
                        ? cloneFlags[1]
                        : (cloneFlags & 2)
          , cloneAttachedData = isArray
                        ? cloneFlags[2]
                        : (cloneFlags & 4)
          ;
        if(cloneIdAndClasses) {
            if(this._id)
                clone.id = this._id;
