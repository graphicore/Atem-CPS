define([
  ...
  , bloomfilter
    ...
    function Instance(pattern, parent, index, data) {
       ...
       if(parent instanceof RootAPI) {
        ...
        this._instancesBasedOnThis = new Set();
    ...
    _p.cpsGet = function(key) {
        var [found, result] = whitelisting.checkGetters(
                                            this._cps_getters, this, key);
        if(!found)
            // if this also fails, we only get the key error of the pattern
            // not the additional error info of this instance, stored
            // in `result` at this point.
            // Good for now, a better handling will probably/eventually
            // don't raise exceptions anymore and support the UI in
            // handling this gracefully.
            return this.pattern.cpsGetFromInstance(this, key);
        return this[result];
    };

    _p.cpsHas = function(key) {
        return (whitelisting.hasMethod.call(this, key)
                        || this.pattern.cpsHasFromInstance(this, key));
    };
    ...
    _p.loadData = function(instanceData) {
    ...
    _p.setBaseInstances = function(instances) {
        ...
        if(changed)
            ...
            // FIXME; // trigger -> the StyldeDict needs cache invalidation
                      // better in updateRegisteredBaseInstances probably!
    };
    ...
    Object.defineProperty(_p, 'pattern', {
        ...
        set: function(pattern) {
            if(pattern === this._pattern)
                ...
                return;
            if(this.pattern && !this.pattern.isEssenceCompatible(pattern))
            ...
            if(!this.isRoot() && this.index === null)
                console.log('no index!:', this);
                //=> we must have an index already
                //
            if(!this.isRoot() && this.parent.pattern.getChild(this.index) !== pattern)
                throw new OMAError('Can\'t set this.pattern as '
                    + pattern + ' it\'s not the same as '
    ...
    _p.unregisterBasedInstance = function(instance){
        this._instancesBasedOnThis.delete(instance);
    };
    ...
    // START High level convenience interfaces to manipulate the pattern
    _p.addNewChild = function(typeName, index) {
        var childPattern = this.pattern.createPattern(typeName);
        return this.addChild(childPattern, index);
    };

    _p.addChild = function(childPattern, index) {
        var canonicalIndex = this.pattern.insertAt(
                            index === undefined ? -1 : index, childPattern);
        // return the new instance
        return this.getChild(canonicalIndex);
    };

    _p.removeChild = function(child) {
        this.pattern.removeAt(child.index, 1);
    };
    // END High level convenience interfaces
    ...
    _p.destroy = function() {
        var i, l;
        assert(this.parent.destoyed
                        // if this is the topmost item to be destroyed
                        || this.parent.find(this) === false
                    , this + ' must not be registered '
                                             + 'anymore in its parent.');
        Object.defineProperty(this, 'destoyed', {value: true});
        this._unsubscribeFromStyleChange(this);

        // must unregister base instances
        // this doesn't remove the base instance references from instanceData
        // so, that can be used after a delete.
        for(let instance of this._registeredBaseInstances)
            instance.unregisterBasedInstance(this);
        this._registeredBaseInstances.clear();
   ...
   _p.requestSetId = function(descendant, id) {
    ...
    _p.addSubtreeIds = function(descendant) {
        ...
        while((node=nodes.pop())) {
            ...
            if(!node.idManager)
                ...
                Array.prototype.unshift.apply(nodes, node.children.slice().reverse());
        }
    };
    ...
    _p._instanciateChild = function(idx) {
        ...
        return new Instance(pattern, this, idx);
    };
    ...
    _p.updateIndex = function() {
        ...
        assert(this.parent.pattern.getChild(index) === this.pattern,
                                        'Parent must agree on pattern');
