  ...
  , bloomfilter
   ...
   _p.requestSetId = function(descendant, id) {
    ...
    _p.query = function(selector) {
        var rootAPI = this.isRoot()
            ? this._rootAPI
            : this.root
            ;

        return rootAPI.query(selector, this);
    };
    ...
    _p.queryAll = function(selector) {
        var rootAPI = this.isRoot()
            ...
            : this.root
            ;
        return rootAPI.queryAll(selector, this);
    };
    ...
    _p.getComputedStyle = function() {
        var rootAPI = this.isRoot()
            ...
            : this.root
            ;
        return rootAPI.getComputedStyleFor(this);
    };

    _p._removeFromTree = function(item) {
        return this.root.removeFromTree(item);
    };

    _p.removeFromTree = function(item) {
        if(!this.isRoot())
            throw new OMAError('Can\'t call removeFromTree on a none-root node.');
        return this._rootAPI.removeFromTree(item);
    };
