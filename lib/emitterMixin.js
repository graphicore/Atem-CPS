define([
    ...
    var EmitterError = errors.Emitter
      , DeprecatedError = errors.DeprecatedError
      ;
    /**
     * usage:
     *
    ...
    function _mixinOn(stateProperty) {
        return function on(channelKey, callback, subscriberData) {
            ...
            subscriberID = _on(state, channelKey, callback, subscriberData);

            off = _off.bind(null, state, subscriberID);
            // for debugging and special cases/old api
            // see Instance for usage
            off.subscriberID = subscriberID;
            return off;
        };
    }
    function _off(state, subscriberIDOrFunc) {
        var i, l
          , subscriberID = typeof subscriberIDOrFunc === 'function'
                    ? subscriberIDOrFunc.subscriberID
                    : subscriberIDOrFunc
          ;

        if(typeof subscriberIDOrFunc === 'function') {
            console.trace();
