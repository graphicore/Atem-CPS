define([
  ...
  , _Node
    ...
    function Instance(){
        this._edge = null;
        // this.parent.pattern === this._edge.from
        this._parent = null;
    }
    var _p = Instance.prototype;
    _p.constructor = Instance;

    _p.setEdge = function(edge) {
        this._edge = edge;
    };

    Object.defineProperties(_p, {
        edge: {
            get: function(){return this._edge;}
          , enumerable: true
        }
      , pattern: {
            get: function(){return this._edge.to;}
          , enumerable: true
        }
      , parent: {
    ...
    _p.setParent = function(parentInstance) {
        this._parent = parentInstance;
    };

    // see OMA._Node for this
    ...
    // connects the instance with the pattern context
    var Edge = (function() {
        function Edge(fromPattern, toPattern, instance) {
            this._from = fromPattern;
            this._to = toPattern;
    ...
    _p._initPattern = function(pattern, nodes) {
        ...
        if(Constructor.$inject) {
            ...
            result = this._checkInjectNodes(pattern, nodes);
            if(!result[0]) throw result[1];
...
//
    ...
    _p._instanciateChildren = function(parentInstance) {
        var from = parentInstance.edge.to
          , toPatterns = from.toPatterns
          , i, l, to
          , instance, edge
          ;
        for(i=0,l=toPatterns.length;i<l;i++) {
            to = toPatterns[i];
            instance = new Instance();
            // NOTE: maybe the instance arg is not needed, as this edge
            // is created only for that instance anyways and stored within
            // the instance!
            edge = new Edge(from, to, instance);
            // instance.setPattern(to); === edge.to
            instance.setEdge(edge);
            instance.setParent(parentInstance);
            this._instanciateChildren(instance);
        }
    };
    ...
    _p._instanciate = function() {
        ...
        var rootInstance = new Instance()
          , edge
          ;
        edge = new Edge(null, this._root, rootInstance);
        rootInstance.setEdge(edge);
        rootInstance.setParent(null);
        this._instanciateChildren(rootInstance);
