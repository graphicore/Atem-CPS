define([
  ...
  , StyleDict
    ...
    /*globals setTimeout:true, cancelTimeout:true*/
    ...
    function Controller(ruleController, rootNodeFactory, selectorEngine) {
        ...
        // {element.nodeID: [styleDict, elementSubscription]}
    ...
    _p.getRulesForElement = function(element) {
        var ruleKey = this.getCPSName(element)
          ...
          , rules
          ;
        // An element needs not to have any rules attached to it
        if(ruleKey === null)
            return [];
    ...
    _p._updateRule = function(ruleKey) {
        var elements = this._rules[ruleKey][2]
          , styleDict
          , i, l
          ;
        for(i=0,l=elements.length;i<l;i++) {
            // FIXME: It seems to me hat element should not be in
            // this._rules[ruleKey][2] if it has no root!
            // Why did purgeNode not remove this item?
            if(!elements[i].root) continue;

            styleDict = this.getComputedStyle( elements[i] );
            styleDict.checkRules();
        }
    };
    ...
    _p.purgeNode = function(node) {
        var nodes = [], element = node, ruleKey, rule, idx, elementData;
        if(node.parent)
            throw new CPSError('Can\'t purge when node has a parent.');
        do {
            // remove the styleDict and all references to it
            ruleKey = this.getCPSName(element);
            rule = this._rules[ruleKey];
            idx = rule[2].indexOf(element);
            if(idx !== -1)
                rule[2].splice(idx, 1);
