define([
  ...
  , parseRules
    ...
    function Controller(ruleController, rootNodeFactory, selectorEngine) {
        ...
        // {ruleKey:[propertyCollection, subscriptionID, [element.nodeIDs, ...]]}
    ...
    _p.purgeNode = function(node) {
        var nodes = [], element = node, ruleKey, rule, idx, styleDict;
        if(node.parent)
        ...
        do {
            ...
            idx = rule[2].indexOf(element.nodeID);
            if(idx !== -1)
                rule[2].splice(idx, 1);
            styleDict = this._styleDicts[element.nodeID];
            if(styleDict) {
                styleDict.invalidateRules();
                delete this._styleDicts[element.nodeID];
            }
