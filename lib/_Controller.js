define([
  ...
  , StyleDict
    ...
    /**
     * TODO: rename to CPSController
     */
    function Controller(ruleController, rootInstance) {
        this._ruleController = ruleController;
        this._selectorEngine = ruleController.selectorEngine;
        this._root = rootInstance;
    ...
    _p.purgeNode = function(node) {
        ...
        if(node.parent.find(node) !== false)
            throw new CPSError('Can\'t purge when node ' + node
                             + ' is still registered in parent.');
        do {
